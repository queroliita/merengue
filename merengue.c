  ...
  if (bpnb!=NULL)
    for ( int i = 0 ; i < bpnb->len ; ++i ) {
      if ( iskey(bpnb->word[i]) ) {
        ...
        else if ( type == 0 ) { set0(&Xg[0],bpnb->word[i],bpnb->bit[i]); set1(&Xg[1],bpnb->word[i],bpnb->bit[i]); }
...
static int measurement(char *flag, ECRYPT_ctx Xf[2], ECRYPT_ctx Xg[2], int i, int j, int rounds) {
  ...
  else if ( same(flag,"back") ) return backflip(Xf,i,j);
  else if ( same(flag,"for") )  return forflip(Xf,i,j);
  return 0;
}
